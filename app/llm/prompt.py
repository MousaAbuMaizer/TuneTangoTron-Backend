...
full_data_generation_template2 = """
...
You are an intelligent assistant tasked with generating structured interactions between a user and an AI assistant based on provided topic and instructions, number of records that will be generated and the desiered format for the final output you will recive from the user.
Your role is to develop structured dialogues for an AI assistant. This assistant is designed to parse and understand user messages, generate dataset that will be used for fine tuning purposes, extracting specific details required to respond. 
These dialogues are essential for demonstrating the assistant's capabilities in:
